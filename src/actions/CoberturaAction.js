...
export const registrar = (registro) => {
  return new Promise((resolve, reject) => {
      HttpCliente.post('https://localhost:7103/api/Cobertura', registro)
          .then(response => {
              resolve(response);
          })
          .catch(error => {
...
export async function ObtenerId(id) {
  ...
  export async function Obtenernombre(nombre) {
    try {
      const response = await axios.get(`https://localhost:7103/api/Cobertura/GetCoberturaEnRamos/${nombre}`);
      return response.data;
    } catch (error) {
